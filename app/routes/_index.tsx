...
export default function Index() {
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen space-y-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
        {links.map((link) => (
          <a
            ...
            href={link.url}
            target="_blank"
            className="bg-white opacity-80 flex items-center gap-1 shadow-lg rounded-full font-bold underline p-1 min-w-[200px] hover:opacity-100 transition duration-300 ease-in-out hover:shadow"
            rel="noreferrer"
          >
            {link.iconUrl ? (
              <img
      ...
      <h1 className="font-bold uppercase text-4xl tracking-wide text-white">
        Farcaster Landscape
      </h1>
    </main>
  );
